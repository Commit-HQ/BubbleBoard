...
Messages live under **Messages** in the app header. Each inquiry has an encrypted subject and a separate conversation. The inbox shows the latest message, unread state, classroom and (for staff) family, with subject search, open/closed filters, and more inquiries on demand. Older conversation messages load in batches of 50.

An admin sets **Parent messaging** on each classroom's management page: an on/off switch, one interval per weekday, and the monthly allowance of new inquiries per family. Messaging starts disabled with a zero allowance; configure and enable it before inviting parents to use it. Saturday and Sunday are closed. Hours and calendar months use Europe/Zagreb, including daylight saving. Holidays are not detected; the interface explains that replies may take longer on non-working days.

Only a new inquiry started by a family uses its monthly allowance. Replies, and inquiries started by teachers, do not. The switch and hours still apply to every family message. Teachers may send at any time and close inquiries; closed inquiries remain readable. All devices of a family share the allowance and read state; each teacher has their own read state. Assigned teachers receive notifications, and all devices of the family receive replies, when push is enabled. Admins can access all classrooms, as elsewhere in the app, but receive pushes only for classrooms they teach.
