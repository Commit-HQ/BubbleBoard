...
- Staff and family cards, recovery cards included, share this format. Only the server's records say what a card is and whether it belongs to an admin. No card expires by itself: there are no temporary or substitute cards, so a substitute gets an ordinary staff card that an admin removes afterwards (product-spec.md, Current decisions).
- Replacing a card is always a reset, never a quieter rotation: the card's sessions end with it, so a family's devices all connect again with the new card, and its one-time cards end too. The Family Key stays as it was, so nothing already encrypted has to be written again. Only admins replace family cards.
...
- Staff are kept to their classrooms and roles by server authorization only (see Keys). Since 2026-09-20 only admins may replace family cards: this closes the cross-classroom family-card replacement path before event photos. The shared Staff Key remains a documented trust boundary. Event photo contexts and grants are specified in [events-format.md](events-format.md).
