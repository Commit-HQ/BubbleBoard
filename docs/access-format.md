...
| Group Key  | classroom                                   | the classroom profile (its name), the Notice Key of each of its notices, the Info Key, the photo of its board with its details, and the Event Key of each of its events                                                   |
| Family Key | family                                      | the Group Key of each classroom its children are in, its answers to polls whose counts families don't see, its private inquiry subjects and messages, and its children's names, photo consent, and face patches in events |
...
| File Key   | file on a notice, info page, or inquiry     | the file's bytes                                                                                                                                                                                                          |
| Poll Key   | poll whose counts families see              | every family's answer to that poll                                                                                                                                                                                        |

Every staff card, whatever its holder's role, opens the same Staff Key. Which classrooms a staff member sees, and what her role lets her change, is decided by server authorization, not encryption. A teacher who also holds a copy of the database, or a server bug that serves another classroom's records, could therefore decrypt that classroom, and a lost staff card together with a database copy exposes the whole kindergarten. Whoever runs the server still reads nothing, and families stay separated by encryption. Separating staff by encryption too would take a Head Key, a key for each staff member, and a classroom key for each, re-wrapped in a head's browser at every change of role or assignment.
...
- Staff and family cards, recovery cards included, share this format. Only the server's records say what a card is and which role it belongs to. No card expires by itself: there are no temporary or substitute cards, so a substitute gets an ordinary staff card that the head removes afterwards (product-spec.md, Current decisions).
- Replacing a card is always a reset, never a quieter rotation: the card's sessions end with it, so a family's devices all connect again with the new card, and its one-time cards end too. The Family Key stays as it was, so nothing already encrypted has to be written again. Only a head, or a group lead of one of the family's classrooms, replaces family cards.
...
| `device-name`               | who uses one of a family's devices                                                              | the family's Family Key            | `null`       | device ID     |
...
| `notice-file`               | a file on a notice, an info page, or an inquiry's message: a document, or a picture             | its File Key                       | `null`       | file ID       |
...
- Staff are kept to their classrooms and roles by server authorization only (see Keys). Only a head, or a group lead of one of the family's classrooms, may replace family cards, which keeps the cross-classroom replacement path to staff who run the kindergarten or the group the family is in. A lead can replace the card of a family that also has a child elsewhere, so that path is narrowed, not closed; the owner accepted it so a lead need not fetch the head to hand a parent a new card. The shared Staff Key remains a documented trust boundary. Event photo contexts and grants are specified in [events-format.md](events-format.md).
...
- A family's devices see the family's connected devices, each with the name someone gave it (`device-name`, JSON `{ name }`, with the device's ID as its subject), and any of them signs out any other. The ID is the session's own, because the hash of its token stays on the server. Staff hold every Family Key, so only server authorization keeps the names to the family: no route gives them to staff. Removing a device ends its session and nothing else: it connects again with the family's card, so keeping someone out still takes replacing the card. A name says what whoever typed it chose, and nothing checks it.
...
A teacher can attach documents and pictures to a message. Each file is encrypted with a File Key of its own, which travels inside the message's envelope beside the file's name (purpose `notice-file`, as on notices), so whoever opens the message opens its files; the server keeps the bytes in R2 under `messages/<message>/<file>`, counts them against the storage limits, and knows only which message carries which file. Only staff upload: a family's message carries its words alone, and the server refuses a family that names a file. A file goes when the message's inquiry goes, and deleting a closed inquiry deletes its bytes at once.
...
The server authorizes every conversation read and write by current family membership and teacher classroom access (a head reaches every classroom). That authorization is an API boundary; the existing shared Staff Key is not a cryptographic isolation boundary between staff members. Only opaque routing/author IDs, the month a message spent from the family's allowance, timestamps, closed state, policy settings and read sequences are stored in plaintext. Notifications contain no subject or message text.
...
Invitations are a snapshot of children and family cards at publication. New children or newly linked family cards are included in the next offer. Child changes revoke invitations for removed family cards, including when they still belong to the classroom through another child. Child writes require an explicit `meetingFamilies` list; an older client that omits it is rejected before any write, rather than cancelling invitations as if the list were empty. Moving/deleting a child releases their bookings; removing classroom membership revokes its invitations. When the final invitation for a child goes, their reservation is released. Past offers and their invitations are deleted by daily cleanup 90 days after their last slot.
