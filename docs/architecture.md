...
App pages are prerendered once, so a record's ID travels in the query (`/app/child?id=…`) and pages read it in the browser; IDs aren't secret, and the language switch keeps the query. The `(app)` layout creates one `App` state for every app page through context, and starts it after the first navigation, once the router can remove a card from the address bar. App pages keep the title "BubbleBoard", so tab titles and browser history never hold a name.
