...
Primary actions: **+ Cover**, **Undo**, **Done, next photo**, with the final/original switcher under the photo. Redo sits beside Undo. Descriptive metadata stays outside the photo workspace. The photo itself is square-cornered and fills the workspace's width; only the panel around it follows the app's rounded cards.
...
The panel asks **“Who is in the photo?”** and lists the classroom's children, with name search for lists longer than eight. The list takes the room the whole classroom needs rather than scrolling inside the panel, because a scroll inside the page's own scroll is awkward on a phone. Children already labelled in the photo carry a small tick, and say “Already in photo” to a screen reader, but remain selectable: a child may also appear in a mirror. Those already labelled move to a row of their own under the rest, headed “Already in this photo”, so the children still missing from the photo come first; each group is in alphabetical order. That order follows what the teacher has already done, never anything read from the face. Do not suggest identity from a face or automatically copy it from a neighbouring photo.

Choosing a name immediately saves the assignment in the draft and advances to the next unresolved face. The chip of the chosen child shows it at once, and the name is written on the cover, so no status line repeats it; Undo restores both the assignment and the previous selection. After the last face, the teacher reviews the whole photo rather than automatically moving to the next photo.
...
- **Invisible cover:** for a child who is in the photo but can't be recognised, such as one seen from behind. It still needs the child's name, but nothing is drawn over the photo there, so the child shows as they are to everyone, whatever the consent says: the teacher is saying there is no face to protect. The child's family still gets the photo marked as one their child is in. In the editor it is a dashed outline with no sticker; keeping the face covered, alone or with the rest, makes it a real cover again.
- **Remove cover** (trash icon, in the panel's top corner beside the crop it removes, so a phone doesn't put it below the whole classroom's names): remove an incorrect detection as a secondary, undoable action. Avoid an ambiguous “Delete” label.
...
The first tap selects a sticker. Dragging an already selected sticker moves it; a handle on any of its four corners resizes it, keeping the opposite corner where it is. Each handle's small dot carries an invisible circle about a thumb wide on the screen, whatever the zoom and however large the photo is; on a cover too small for four of those, they shrink to a third of it rather than swallow the middle, which stays draggable. The shape stays the same, with a minimum size and a clear boundary around the covered area. A cover may hang off the photo's edge, for a child standing at it, as far as its centre: only the part on the photo is painted and published, and a detected face at the edge gets its padding past the edge rather than clipped away. Both detected and manual covers can shrink to 4 pixels per axis in the working image. Assigning a child does not resize a cover.

The photo uses the editor's full width. + Cover, the selected cover's sticker and the undo/redo icons sit above it, with the zoom slider beside them only where there is a mouse, since a phone zooms with two fingers on the photo itself; the final/original switcher and child assignment sit immediately below. A cover is the ellipse inside the box that is dragged and resized, so the photo shows at the box's four corners, which a detected cover's padding keeps clear of the face itself. There are no separate size sliders or movement buttons. Arrow keys move a cover; Alt + arrow keys resize it. A green border means a child is assigned, dark ink means keep covered, and apricot means a face still waiting; a dark under-stroke keeps each of them readable over any photo. Sticker rotation and freehand masks are outside the first release.

Drag outside the selected sticker to pan the photo. Two fingers zoom the image, never the sticker, around the point between them, so the bit of photo under the fingers stays under them and moving both fingers together slides the photo along. Choosing a child moves on to the next face, and if that one is off the screen while zoomed in, the view goes to it; panning by hand is never overruled a moment later. Starting a two-finger gesture cancels an active sticker drag. The photo is kept out of the browser's scroll anchoring: with the photo partly scrolled off the top of the window, Chrome would otherwise hold the first cover still on screen by scrolling the page as it is dragged, and the page would run away under the pointer. Viewport and zoom changes do not alter image-space protection coordinates.
...
**+ Cover** places a generously sized sticker at the centre of the current view and immediately selects it for movement and assignment. Zooming into a missed face before adding the cover makes this easier. Guests and adults can remain covered.
...
Include detection, name assignment, a private identification crop, adding/moving/resizing covers, original view, undo/redo, gallery review and family previews. Eighteen built-in stickers are available, and new covers, detected or added, take one at random that no cover on the photo wears yet, until all eighteen are used, so a group photo isn't a wall of one face; choosing artwork is optional, so the whole set waits behind one button in the toolbar, beside + Cover, showing the selected cover's current sticker, and opens on a row under the toolbar and puts itself away once one is picked. An invisible cover wears no sticker, so the button rests while one is selected. A later sticker pack must not change the security mask.
