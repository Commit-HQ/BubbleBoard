...
> **Current decisions.** This specification is the original target. The decisions below replace or settle parts of it; where they differ, follow them. The [decisions](decisions.md) record what replaced them since, and the [architecture notes](architecture.md) hold the conventions.
>
...
> - **Seen marks and polls** (extend §6 and §44): families mark notices as seen with a tap and answer polls on them, so teachers know who saw a notice and what each family chose. The server stores which family marked or answered which notice; each answer is encrypted with the family's Family Key, which staff also hold, or, for a poll whose counts families see, with a key inside the notice, so its families count the answers without seeing who gave them. The [decisions](decisions.md) records the decisions.
> - **Board photos** (depart from §20 and §42): each classroom shows one photo of its corkboard, encrypted with the classroom's Group Key and kept in private R2. It stays until it's replaced or taken down rather than for a chosen number of days, and doesn't go through the consent workflow, because it shows the board with no children in it. The [decisions](decisions.md) records the decision.
> - **Files on notices and storage limits** (extend §6, §21, and §43): a notice carries files, each encrypted with a File Key of its own that travels inside the notice's envelope, and they go with the notice. The server counts what it keeps in R2 and refuses uploads and downloads past limits set in `.env`, so an installation stays within R2's free allowance. The [decisions](decisions.md) records the decisions.
> - **Adding a family's devices** (extends §7 and §32): a connected family device can show a one-time card that connects one more of the family's devices within 24 hours, without the printed card, and signs no one out; replacing the family's card still signs out every device. The [access format](access-format.md) and the [decisions](decisions.md) record it.
> - **A family's connected devices** (extends §7 and §32): a family device lists the devices connected for its family, each with the name someone on it gave it, encrypted with the Family Key and given to the family's devices alone, and signs out any of the others. A removed device connects again with the family's card; replacing the card is still what keeps someone out. The [access format](access-format.md) and the [decisions](decisions.md) record it.
> - **Info pages** (extend §53–54): heads write pages for the whole kindergarten, each with text on a paper colour and files, in the order they choose, which every connected device opens from the header, whatever its classrooms, and which stay until they're changed or deleted. They're encrypted with an Info Key of their own, wrapped for the Staff Key and for every classroom's Group Key. The [access format](access-format.md) and the [decisions](decisions.md) record it.
> - **Event photos** (settle §18–28, §52.2, and §59): classroom photos are published as an **event** — a title, a date, a description, and a gallery for one classroom. A teacher's device detects faces locally, the teacher names each covered face or keeps it covered, and reviews the real result before anything is uploaded (§52.2 as written). The safe base has every marked rectangle overwritten before it is encoded; each face travels as its own lossless patch with a key of its own, and the overlaps of several faces as separate patches, rather than as the feathered WebP of §25, because the exact zeroes at a patch's edges are what hold the boundary. An event keeps its published consent snapshot and lasts the days its teacher chose. See [event format](events-format.md).
> - **Photo consent belongs to the family, and staff record the form** (replaces §19 and §28): the choice is per child and encrypted with each linked family's Family Key, not with the Staff Key, so families change it themselves in the app and every linked family must allow sharing before a face is shown to the classroom. Because the paper consent form is still what a kindergarten holds, a head or the classroom's group lead sets the same choice from the child's page and when a child is added, over the row revision they read, so a change a parent made meanwhile is refused rather than overwritten. Missing, unreadable, or contested consent stays private.
...
> - **No temporary or substitute cards** (removes the Substitute QR of §9, the credential expiry of §32.2 and §51.4, and invariants 19 and 34): a card is either valid or replaced. A substitute gets an ordinary staff card, which a head removes afterwards; no card carries an `expires_at`, so no session can outlive one.
> - **Replacing a card is always a reset** (replaces the rotation of §8.1 and invariant 26): a new card means new sessions. Replacing a family's card signs out every device of that family, one-time cards included, and replacing a staff card signs out that card's devices; the Family Key itself doesn't change, so nothing has to be encrypted again. Only a head or a group lead of one of the family's classrooms replaces a family's card.
> - **Files on private inquiries** (settle the family-private documents of §3 and §6.3): a teacher attaches documents and pictures to a message in an inquiry, each encrypted with a File Key of its own inside the message's envelope, as a notice's files are. Families attach nothing; the server refuses it. A file goes when its inquiry does, and its bytes count against the same storage limits.
> - **Changing a message in an inquiry** (extends the private messages of §3 and §6.3): a teacher changes the words of their own messages in an open inquiry and deletes them, as long as the family hasn't answered them, which leaves a “message deleted” placeholder so the conversation's order and both sides' read progress are untouched, and frees the files the message carried. A family changes its own message until a teacher has opened the conversation as far as it or answered it, and never deletes one; an edit costs no inquiry and neither is notified. See the Private inquiries section of the [decisions](decisions.md).
...
> - **Deployment** (§46–47) is run by hand, as the [hosting](hosting.md) guide describes. CI only validates.
