...
> **Current decisions (2026-09-21).** This specification is the original target. The decisions below replace or settle parts of it; where they differ, follow them. The [decisions](decisions.md) record what replaced them since, and the [architecture notes](architecture.md) hold the conventions.
>
...
> - **Build order** (replaces the pilot steps in §60). Built in this order: teacher setup and family enrollment; an encrypted notice with push notifications; privacy-aware photos; then private messages. Files on notices came ahead of photos, at the owner's request (2026-09-13).
> - **Private inquiries (2026-09-18):** titled, family-private conversations are implemented ahead of classroom photos, and read as a chat. Per-classroom admin settings control parent sending (on/off and weekday hours) and the monthly number of inquiries a family may spend, three by default. A family spends one on every message a teacher hasn't answered yet, so an open conversation can't be written in without limit; answering a teacher is free, as is everything a teacher sends. Teachers close conversations. See the Private inquiries section of the [decisions](decisions.md).
> - **Notices** (extends §6, §34, §39–40, and §53): a notice can go to several classrooms, encrypted once with its own Notice Key, which each classroom's Group Key opens. Pushes carry no content. On phones and tablets, BubbleBoard works only once installed, and computers use the browser; notifications are turned on from home. The [decisions](decisions.md) records the decisions.
> - **Seen marks and polls** (extend §6 and §44, 2026-09-13): families mark notices as seen with a tap and answer polls on them, so teachers know who saw a notice and what each family chose. The server stores which family marked or answered which notice; each answer is encrypted with the family's Family Key, which staff also hold, or, for a poll whose counts families see, with a key inside the notice, so its families count the answers without seeing who gave them (2026-09-13). The [decisions](decisions.md) records the decisions.
> - **Board photos** (depart from §20 and §42, 2026-09-13): each classroom shows one photo of its corkboard, encrypted with the classroom's Group Key and kept in private R2. It stays until it's replaced or taken down rather than for a chosen number of days, and doesn't go through the consent workflow, because it shows the board with no children in it. The [decisions](decisions.md) records the decision.
> - **Files on notices and storage limits** (extend §6, §21, and §43, 2026-09-13): a notice carries files, each encrypted with a File Key of its own that travels inside the notice's envelope, and they go with the notice. The server counts what it keeps in R2 and refuses uploads and downloads past limits set in `.env`, so an installation stays within R2's free allowance. The [decisions](decisions.md) records the decisions.
> - **Adding a family's devices** (extends §7 and §32, 2026-09-14): a connected family device can show a one-time card that connects one more of the family's devices within 24 hours, without the printed card, and signs no one out; replacing the family's card still signs out every device. The [access format](access-format.md) and the [decisions](decisions.md) record it.
> - **A family's connected devices** (extends §7 and §32, 2026-09-21): a family device lists the devices connected for its family, each with the name someone on it gave it, encrypted with the Family Key and given to the family's devices alone, and signs out any of the others. A removed device connects again with the family's card; replacing the card is still what keeps someone out. The [access format](access-format.md) and the [decisions](decisions.md) record it.
> - **Info pages** (extend §53–54, 2026-09-14): admins write pages for the whole kindergarten, each with text on a paper colour and files, in the order they choose, which every connected device opens from the header, whatever its classrooms, and which stay until they're changed or deleted. They're encrypted with an Info Key of their own, wrapped for the Staff Key and for every classroom's Group Key. The [access format](access-format.md) and the [decisions](decisions.md) record it.
> - **Event photos** (settle §18–28, §52.2, and §59, 2026-09-19/20): classroom photos are published as an **event** — a title, a date, a description, and a gallery for one classroom. A teacher's device detects faces locally, the teacher names each covered face or keeps it covered, and reviews the real result before anything is uploaded (§52.2 as written). The safe base has every marked rectangle overwritten before it is encoded; each face travels as its own lossless patch with a key of its own, and the overlaps of several faces as separate patches, rather than as the feathered WebP of §25, because the exact zeroes at a patch's edges are what hold the boundary. An event keeps its published consent snapshot and lasts the days its teacher chose. See [event format](events-format.md).
> - **Photo consent belongs to the family, and staff record the form** (replaces §19 and §28, 2026-09-19/20): the choice is per child and encrypted with each linked family's Family Key, not with the Staff Key, so families change it themselves in the app and every linked family must allow sharing before a face is shown to the classroom. Because the paper consent form is still what a kindergarten holds, an admin sets the same choice from the child's page and when a child is added, over the row revision they read, so a change a parent made meanwhile is refused rather than overwritten. Missing, unreadable, or contested consent stays private.
> - **Local face detection is part of the first version** (replaces the V2 deferral in §61): a pinned BlazeFace model runs in a Web Worker on the teacher's device, and no embedding or recognition service is involved. Detection is assistive; the manual covers and the review before publishing remain required.
> - **No classroom roster for families** (departs from §6.2 and §29, and from invariant 6): child names stay under the Staff Key, and a family's own children's names are projected for its Family Key alone. Families see their own children, never a list of the classroom's.
> - **No temporary or substitute cards** (removes the Substitute QR of §9, the credential expiry of §32.2 and §51.4, and invariants 19 and 34, 2026-09-21): a card is either valid or replaced. A substitute gets an ordinary staff card, which an admin removes afterwards; no card carries an `expires_at`, so no session can outlive one.
> - **Replacing a card is always a reset** (replaces the rotation of §8.1 and invariant 26, 2026-09-21): a new card means new sessions. Replacing a family's card signs out every device of that family, one-time cards included, and replacing a staff card signs out that card's devices; the Family Key itself doesn't change, so nothing has to be encrypted again. Only admins replace family cards.
> - **Files on private inquiries** (settle the family-private documents of §3 and §6.3, 2026-09-21): a teacher attaches documents and pictures to a message in an inquiry, each encrypted with a File Key of its own inside the message's envelope, as a notice's files are. Families attach nothing; the server refuses it. A file goes when its inquiry does, and its bytes count against the same storage limits.
> - **Changing a message in an inquiry** (extends the private messages of §3 and §6.3, 2026-09-22): a teacher changes the words of their own messages in an open inquiry and deletes them, as long as the family hasn't answered them, which leaves a “message deleted” placeholder so the conversation's order and both sides' read progress are untouched, and frees the files the message carried. A family changes its own message until a teacher has opened the conversation as far as it or answered it, and never deletes one; an edit costs no inquiry and neither is notified. See the Private inquiries section of the [decisions](decisions.md).
> - **Media retention** (§42) is decided as written: 1, 3, 7, 14, 30, 60, or 90 days, with 30 days as the default for classroom photos and notices. A classroom's board photo and an info page's files are the exceptions: they stay until they're replaced, changed, or deleted.
...
> **Superseded:** see the build order in *Current decisions* at the top.
