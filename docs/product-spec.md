...
> - **Info page** (extends §53–54, 2026-09-14): admins write one info page for the whole kindergarten, with text and files, which every connected device opens from the header, whatever its classrooms, and which stays until it's changed. It's encrypted with an Info Key of its own, wrapped for the Staff Key and for every classroom's Group Key. The [access format](access-format.md) and the [next step plan](next-step-plan.md) record it.
