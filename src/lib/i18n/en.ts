...
export const en = {
	...
	app: {
		...
		eventEditor: {
			...
			intro:
				'Prepare and review photos on this device. Publishing events and family visibility settings are coming next; no photos are uploaded here.',
			local: 'This draft stays in this open page. Closing or reloading the app loses it.',
			classroom: 'Classroom',
			add: 'Add photos',
			...
			noFaces: 'No faces found. Check the whole photo and add any missing covers.',
			photo: (n: number, total: number) => `Photo ${n} of ${total}`,
			face: (n: number) => `Face ${n}`,
			remaining: (n: number) => `${n} faces still need a name or a cover decision.`,
			addCover: 'Add cover',
			original: 'Show original',
			covers: 'Restore covers',
			originalHint: 'Original — editing only',
			...
			coveredDone: 'This face will stay covered.',
			check: 'Check the whole photo, including faces detection may have missed.',
			reviewed: 'Reviewed',
			...
			previewHint:
				'Every marked face is covered in this flattened preview. Family-specific reveals and publishing are not connected yet.',
			previewFailed: 'Could not prepare the preview. Return to editing and try again.',
			unusable: 'This photo could not be opened. Choose a JPEG, PNG, WebP or HEIC photo.',
			leave: 'Leave this page and discard the photo draft?',
			allReviewed: 'All photos reviewed. You can inspect the covered results below.',
			gesture:
				'Select a cover, then drag it or its corner. Use zoom and drag the photo background to inspect details. Arrow keys move the selected cover.',
			tooManyFaces: 'This photo has too many covers. Use another photo or remove false detections.',
			rosterChanged: 'The classroom roster changed. Check the labels again.'
		},
