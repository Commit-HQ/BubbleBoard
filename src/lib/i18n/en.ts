...
export const en = {
	...
	app: {
		eventEditor: {
			title: 'Event photo editor', open: 'Prepare event photos', intro: 'Prepare and review photos on this device. Publishing events and family visibility settings are coming next; no photos are uploaded here.',
			local: 'This draft stays in this open page. Closing or reloading the app loses it.', classroom: 'Classroom', add: 'Add photos', limit: 'Choose up to 20 photos, each no larger than 50 MB.',
			loading: 'Preparing photos…', detecting: 'Looking for faces…', failed: 'Face detection is unavailable. Add covers manually and check the whole photo.', retry: 'Retry detection', manual: 'Continue manually', noFaces: 'No faces found. Check the whole photo and add any missing covers.',
			photo: (n: number, total: number) => `Photo ${n} of ${total}`, face: (n: number) => `Face ${n}`, remaining: (n: number) => `${n} faces still need a name or a cover decision.`,
			addCover: 'Add cover', original: 'Show original', covers: 'Restore covers', originalHint: 'Original — editing only',
			who: 'Who is in this photo?', crop: 'Original crop — visible only while editing', search: 'Find a child', already: 'Already in this photo', empty: 'No children match.', covered: 'Keep covered', unknown: 'Choose a child', removeCover: 'Not a face — remove cover',
			undo: 'Undo', redo: 'Redo', zoom: 'Zoom photo', width: 'Cover width', height: 'Cover height', left: 'Move left', right: 'Move right', up: 'Move up', down: 'Move down',
			assigned: (name: string) => `Assigned: ${name}`, coveredDone: 'This face will stay covered.', check: 'Check the whole photo, including faces detection may have missed.', reviewed: 'Reviewed', review: 'Reviewed, next photo', reviewNeeded: 'Needs review',
			overlap: 'Some covers overlap. Check their edges carefully. This preview keeps all faces covered.', removePhoto: 'Remove photo',
			preview: 'Review covered photos', back: 'Back to editing', previewHint: 'Every marked face is covered in this flattened preview. Family-specific reveals and publishing are not connected yet.', previewFailed: 'Could not prepare the preview. Return to editing and try again.',
			unusable: 'This photo could not be opened. Choose a JPEG, PNG, WebP or HEIC photo.', leave: 'Leave this page and discard the photo draft?', allReviewed: 'All photos reviewed. You can inspect the covered results below.',
			gesture: 'Select a cover, then drag it or its corner. Use zoom and drag the photo background to inspect details. Arrow keys move the selected cover.', tooManyFaces: 'This photo has too many covers. Use another photo or remove false detections.', rosterChanged: 'The classroom roster changed. Check the labels again.',
		},
