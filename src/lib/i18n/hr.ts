...
export const hr = {
	...
	app: {
		...
		eventEditor: {
			...
			intro:
				'Pripremi i pregledaj fotografije na ovom uređaju. Objavljivanje događaja i roditeljske postavke vidljivosti slijede u idućem koraku; ovdje se fotografije ne šalju.',
			local:
				'Nacrt ostaje na ovoj otvorenoj stranici. Zatvaranjem ili ponovnim učitavanjem aplikacije gubi se.',
			classroom: 'Skupina',
			...
			noFaces: 'Nisu pronađena lica. Pregledaj cijelu fotografiju i dodaj pokrove koji nedostaju.',
			photo: (n: number, total: number) => `Fotografija ${n} od ${total}`,
			face: (n: number) => `Lice ${n}`,
			remaining: (n: number) => `Još ${n} lica treba označiti ili ostaviti prekrivenima.`,
			addCover: 'Dodaj pokrov',
			original: 'Prikaži original',
			covers: 'Vrati pokrove',
			originalHint: 'Original — samo za uređivanje',
			who: 'Tko je na slici?',
			...
			unknown: 'Odaberi dijete',
			removeCover: 'Ukloni pokrov',
			undo: 'Poništi',
			redo: 'Vrati poništeno',
			zoom: 'Povećanje fotografije',
			width: 'Širina pokrova',
			height: 'Visina pokrova',
			left: 'Pomakni lijevo',
			right: 'Pomakni desno',
			up: 'Pomakni gore',
			down: 'Pomakni dolje',
			...
			check: 'Pregledaj cijelu fotografiju, uključujući lica koja detekcija možda nije pronašla.',
			reviewed: 'Pregledano',
			...
			previewHint:
				'U ovom sastavljenom pregledu sva označena lica ostaju prekrivena. Otkrivanje prema obitelji i objava još nisu povezani.',
			previewFailed: 'Pregled nije moguće pripremiti. Vrati se na uređivanje i pokušaj ponovno.',
			unusable: 'Fotografiju nije moguće otvoriti. Odaberi JPEG, PNG, WebP ili HEIC fotografiju.',
			leave: 'Napustiti stranicu i odbaciti nacrt fotografija?',
			allReviewed:
				'Sve su fotografije pregledane. U nastavku možeš provjeriti prekrivene rezultate.',
			gesture:
				'Odaberi pokrov pa povuci njega ili njegov kut. Za detalje povećaj fotografiju i povuci njezinu pozadinu. Tipke sa strelicama pomiču odabrani pokrov.',
			tooManyFaces:
