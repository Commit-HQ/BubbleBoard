...
export async function settingsFor(db: D1Database, classroom: string): Promise<MessageSettings> {
	const row = await db
		.prepare(
			'SELECT enabled, monthly_limit AS monthlyLimit, schedule, revision FROM message_settings WHERE classroom_id = ?'
		)
		.bind(classroom)
		.first<{ enabled: number; monthlyLimit: number; schedule: string; revision: number }>();
	return row
		? { ...row, classroom, enabled: !!row.enabled, schedule: JSON.parse(row.schedule) }
		: { classroom, enabled: false, monthlyLimit: 0, schedule: defaultSchedule(), revision: 0 };
}
...
export async function inbox(db: D1Database, who: Identity, now = Date.now()): Promise<Inbox> {
	...
	const policies = await Promise.all(
		classrooms.results.map(async ({ id }) => {
			const settings = await settingsFor(db, id);
			const used =
				who.kind === 'family'
					? await db
							.prepare(
								'SELECT COUNT(*) AS n FROM conversations WHERE classroom_id=? AND family_id=? AND month=? AND starter=?'
							)
							.bind(id, who.family, messageClock(now).month, actor(who))
							.first<{ n: number }>()
					: null;
			return { ...settings, used: used?.n ?? 0, allowed: sendingAllowed(settings, now) };
		})
	);
	const { results } = await db
		.prepare(
			`SELECT c.id,c.family_id AS family,c.classroom_id AS classroom,c.title,c.starter,c.closed,c.created_at AS createdAt,
...
function guard(who: Identity, classroom: string, family: string, now: number, quota: boolean) {
	const [sql, params] = visibleClassrooms(who);
   ...
   AND json_extract(p.schedule,?) <= ? AND json_extract(p.schedule,?) > ?`;
		...
		if (quota) {
			condition += ` AND (SELECT COUNT(*) FROM conversations WHERE classroom_id=? AND family_id=? AND month=? AND starter=?) < p.monthly_limit`;
			values.push(classroom, family, clock.month, actor(who));
		}
		condition += ')';
	}
...
export async function startConversation(
	...
	const existing = await db
		.prepare('SELECT title,starter FROM conversations WHERE id=?')
		.bind(value.id)
		.first<{ title: string; starter: string }>();
	if (existing) {
		const thread = await conversationFor(db, who, value.id);
		const first = await db
			.prepare('SELECT id,content FROM messages WHERE conversation_id=? ORDER BY sequence LIMIT 1')
			.bind(value.id)
			.first<{ id: string; content: string }>();
		if (
			thread.classroom !== value.classroom ||
			thread.family !== value.family ||
			existing.starter !== actor(who) ||
	...
	const { condition, values } = guard(who, value.classroom, value.family, now, true);
	const results = await db.batch([
		db
			.prepare(
				`INSERT INTO conversations(id,family_id,classroom_id,title,starter,month,created_at)
   SELECT ?,?,?,?,?,?,? WHERE ${condition} ON CONFLICT(id) DO NOTHING`
			...
			.bind(
				...
				value.title,
				actor(who),
				messageClock(now).month,
				now,
				...values
			),
		db
			.prepare(
				`INSERT INTO messages(id,conversation_id,author,content,posted_at)
   SELECT ?,?,?,?,? WHERE changes()=1`
			)
			.bind(value.message, value.id, actor(who), value.content, now)
	]);
...
async function explainBlocked(
	...
	if (who.kind === 'family') {
		...
		if (quota) error(409, 'messages-limit');
	}
	error(409, 'messages-closed');
}
export async function reply(
	...
	const { condition, values } = guard(who, thread.classroom, thread.family, now, false);
	const result = await db
		.prepare(
			`INSERT INTO messages(id,conversation_id,author,content,posted_at)
 SELECT ?,?,?,?,? WHERE EXISTS(SELECT 1 FROM conversations WHERE id=? AND closed=0) AND ${condition}
 ON CONFLICT(id) DO NOTHING`
		)
		.bind(message, id, actor(who), content, now, id, ...values)
		.run();
	if (!result.meta.changes) {
		if (await db.prepare('SELECT id FROM messages WHERE id=?').bind(message).first())
			return reply(db, who, id, message, content, now);
		await explainBlocked(db, who, thread.classroom, thread.family, now, false);
	}
	return true;
}
