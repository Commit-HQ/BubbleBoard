<script lang="ts">
	...
	// should know, each on its paper with its files, in the order the head put them in. The head adds pages
	// here, and changes, moves, and deletes each; until there's one, the page says so.
...
<Screen locale={data.locale} title={t.title} need="connected">
	{#if app.head && pages.length + app.unreadableInfoPages < maxInfoPages}
	...
	{:else if !app.unreadableInfoPages}
		<p class="text-muted">{app.head ? t.emptyHead : t.empty}</p>
	{/if}
</Screen>
