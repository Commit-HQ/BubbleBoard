<script lang="ts">
	...
	// The three groups of questions, each with an icon and an anchor of its own.
	const groups = $derived(
		(
			[
				['faq-parents', 'heart', t.faq.groups.parents],
				['faq-teachers', 'users', t.faq.groups.teachers],
				['faq-technical', 'lock', t.faq.groups.technical]
			] as const
		).map(([id, icon, group]) => ({ id, icon, ...group }))
	);
</script>
...
<div class="flex flex-col gap-20 pb-20 lg:gap-28 lg:pb-28">
	...
	>
		...
		<ul class="grid gap-4 sm:grid-cols-2">
			...
			{@render feature('users', t.teachers.meetings)}
			{@render feature('clock', t.teachers.retention)}
			{@render feature('check', t.teachers.seen)}
		</ul>
	</section>
	...
	>
		...
		<div class="lg:sticky lg:top-24 lg:self-start">
			{@render intro('faq', t.faq.title, t.faq.copy)}
		...
		<div class="grid gap-5">
			{#each groups as { id, icon, title, items } (id)}
				<section
					{id}
					class="scroll-mt-24 rounded-4xl glass p-6 sm:p-8"
