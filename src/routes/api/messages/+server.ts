...
import { inbox, messageId, sealed, startConversation } from '$lib/server/messages';
...
export const POST: RequestHandler = async (event) => {
	...
	const value = {
		...
		classroom: messageId(body.classroom),
		family: messageId(body.family),
		title: sealed(body.title, 1000),
		message: messageId(body.message),
